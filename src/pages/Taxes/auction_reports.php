...
$reports="<div style='margin-top:10px;'><select name='report' onchange='report_options(this.value);'>";
...
{
    echo "
    <div class='report-buttons-wrap'></div>
    ";
	  $curs55 = oci_parse($c,"select * from (
                                   select 
									c.extcode,
									t.curr_id,
									t.mf as mfi,
   ...
   
   {
      ...
      case 1: {
		     ...
		     $currency = 'сом';
		  $currencies = 'в сомах';
	       while (oci_fetch($curs55)){
		
		  $page = oci_result($curs55, 2);
		  $shortname2 = oci_result($curs55, 5);
        ...
        if ($c = ora_logon(OP1,OP2))
        {
                ...
                (select distinct ts.makedate(date0) from ls.auction_deals d where d.order_id = t.rowid and d.volume is not null) as dl_date, 
                 (select distinct  ts.makedate(date0) from ls.auction_deals d where d.order_id = t.rowid and d.volume is not null) as dl_date,  
                (select distinct  d.volume from ls.auction_deals d where d.order_id = t.rowid and d.volume is not null) as dl_volume, 
                (select distinct  d.price from ls.auction_deals d where d.order_id = t.rowid and d.volume is not null) as dl_price, 
              ...
              ");
                ...
                oci_execute($curs7);
                $curs8 = oci_parse($c, "select ai.volume,ai.volume_nocomp, (select sum(volume) from ls.auction_orders ao where ao.curr_id=ai.curr_id and ao.date0=ai.date0 and ao.price>0) as svol, 
				(select sum(volume) from ls.auction_orders ao where ao.curr_id=ai.curr_id and ao.date0=ai.date0 and ao.price<0) as sncvol,
				curr.shortname, curr.namerus, ai.pred, ai.tot, ai.repayment_date, ai.freq_payments, 
				ai.settlement_date from ls.auction_issue ai, ".TS.".currinstrument curr where curr.id = ".$page." and  curr.id=ai.curr_id and ai.date0=".MakeLongDate($date1));
                ...
                ".TS.".currinstrument c,ls.auction_issue i where c.id = ".$page." and t.curr_id=c.id and t.curr_id = i.curr_id and t.date0=".MakeLongDate($date1)." and i.date0=".MakeLongDate($date1)." order by shortname ASC,t.price DESC");
                ...
                where c.id = ".$page." and t.curr_id=c.id and date0=".MakeLongDate($date1)." group by ls.auction_make_comp(t.price), c.cupontax");
      ...
      from
      (
      select distinct t.idfirm from ls.auction_orders t, ts.tradeaccount tt where t.curr_id = ".$page." and t.status != 2 and date0=".MakeLongDate($date1)."
      ) tab";
      ...
      $bank_users=" select count(*) from (
         ...
         n.nick 
          ...
          ".TS.".firm n where o.curr_id = ".$page." and o.status != 2 and n.id=o.idfirm and date0=".MakeLongDate($date1)." and n.nick LIKE '%BNK%' group by o.idfirm,
      ...
      if ($cnt_us = ora_logon("ls@".SRV,"ls"))
         {
		  ...
		  $institut_users = $cnt_users - $cnt_users_bank;
		  
          
      $appealPeriod = ''; 
      if(substr($shname,0,5) == "GBA02")
      {$appealPeriod = "2 года";}
      else if (substr($shname,0,5) == "GBA03") {
      {$appealPeriod = "3 года";}
      }
  ...
  echo "
  <div class='report_wrapper' curr_id='".$page."' shortname='".$shortname2."' style='width:700px;margin:10px auto;'> 
<div class='".$page."' id='wrapper-pdf'>
  <table width='100%'>
 <tr>
 ...
 <span style='font-size:15px;'>Общая информация по аукциону:</span>
       <table class='new-report short' border='1'>
         ...
         <table class='new-report' border='1'>
            ...
            <th>По номиналу (".$currencies.")</th>
            <th>По факту (".$currencies.")</th>
  ...
  t.price,
   t.volume*t.price as total,  
  (select distinct (t.volume*round(d.price,2)) from ls.auction_deals d where d.order_id=t.rowid) as dl_total,
  t.profit,
  round((select distinct d.profit from ls.auction_deals d where d.order_id=t.rowid),2) as profit1
  ...
  ".TS.".currinstrument c where  t.curr_id=c.id and t.conf != 2 and t.date0=".MakeLongDate($date1)." order by t.price desc) 
     tt group by
      ...
      tt.price,    
         ...
         oci_execute($curs7);
         $curs8 = oci_parse($c, "select ai.volume,ai.volume_nocomp, (select sum(volume) from ls.auction_orders ao where ao.curr_id=ai.curr_id and ao.date0=ai.date0 and ao.price>0) as svol, 
		 (select sum(volume) from ls.auction_orders ao where ao.curr_id=ai.curr_id and ao.date0=ai.date0 and ao.price<0) as sncvol, curr.shortname,
		 ai.pred, ai.tot from ls.auction_issue ai, ".TS.".currinstrument curr where curr.id = ".$page." and curr.id=ai.curr_id and ai.date0=".MakeLongDate($date1));
  ...
  <table class='new-report' border='1'>
     <tr>
     <th style='width:70px;'>Цена (".$currency.")</th>
     <th>Сумма заявок по номинальной цене (".$currency.")</th>
     <th>Объем поступлений при удовлетворении заявок по данной цене (".$currency.")</th>
...
";
      ...
      case 2: {
         ...
         if ($c = ora_logon(OP1,OP2))
         {
            ...
            else
            {
               ...
               ( select DISTINCT  profit  from  ls.auction_deals d  where d.order_id = t.rowid ) as profit_2
               from ls.auction_orders t, ".TS.".ts_currinstrument_arcts curr, ".TS.".tradeaccount acc, ".TS.".firm f,
			   ls.nicks rn where t.curr_id = ".$page." and t.curr_id=curr.id and t.conf != 2 and t.idfirm=f.id and date0=".MakeLongDate($date1)."
			   and acc.id=t.account ".$f." and f.nick = rn.nick order by comp desc, price desc");
			   ...
			   ai.volume_nbkr+ai.volume_nocomp_nbkr as total_nbkr,ai.volume_nocomp_nbkr
                from ls.auction_issue ai, ".TS.".currinstrument curr where curr.id = ".$page." and curr.id=ai.curr_id and ai.date0=".MakeLongDate($date1)."");
   ...
   <tr>
   <td>Объем выпуска (".$currency.")</td>
   <td>".number_format($allTotal*NamCost, 2, ',', ' ')."</td>
   </tr>
   <tr>
   <td>в т.ч Неконкурентные на сумму (".$currency.")</td>
...
$currency = 'рубль';
      ...
      case 3: {
         ...
         if ($c = ora_logon(OP1,OP2))
         {
                 ...
                 (select distinct  d.volume from ls.auction_deals d where d.order_id=t.rowid and d.volume is not null) as dl_volume,
                 (select distinct  d.price from ls.auction_deals d where d.order_id=t.rowid and d.volume is not null) as dl_price,
                 (select distinct  (d.volume*round(d.price,2)) from ls.auction_deals d where d.order_id=t.rowid and d.volume is not null) as dl_total,
		c.shortname,t.status,  
		case when i.profit_nocomp > 0 then i.profit_nocomp else i.profit_nocomp_avg end as profit,
		t.profit as ord
              from ls.auction_orders t, ".TS.".currinstrument c,ls.auction_issue i 
              where t.status != 2 and c.id = ".$page." and t.curr_id=c.id and t.curr_id = i.curr_id and t.date0=".MakeLongDate($date1)." and i.date0=".MakeLongDate($date1)."
              order by shortname ASC,t.price DESC");
                 ...
                 where curr.id = ".$page." and curr.id=ai.curr_id and ai.date0=".MakeLongDate($date1));
  ...
  <span style='font-size:15px;'>Итоги размещения:</span>
        <table class='new-report' border='1'>
         ...
         <th class='info_title' >
            Сумма удовлетворенных заявок (по номиналу, в ".$currencies.")
         </th>
         <th class='info_title'>
            Сумма удовлетворенных заявок (по факту, в ".$currencies.")
         </th>
         <th class='info_title'>
            Средневзвешенная цена (".$currency.")
         </th>
         <th class='info_title'>
             Доходность (в %)
         </th>
         ...
         //prev
			...
			//   $pred_date = 132582407;
			 ...
			 $pred_date = 132582421;
            $prev__curs7 = oci_parse($c, " select * from (select ls.auction_make_comp(t.price) as comp,
            (select sum(d.volume) from ls.auction_deals d where d.order_id=t.rowid) as dl_volume,
            (select sum((d.volume*round(d.price,2))) from ls.auction_deals d where d.order_id=t.rowid) as dl_total
         from ls.auction_orders t, ".TS.".currinstrument c,ls.auction_issue i 
         ...
         order by i.date0 DESC) where ROWNUM <= 1");
          ...
          /// itogovaya tablica
                    if ($itogi = ora_logon("ls@".SRV,"ls"))
                    {
					  $shortNm = substr($shname,0,3);
						...
						$curs83 = oci_parse($itogi, "select * from (
						               select ai.volume,ai.volume_nocomp,
									  ...
									  ai.date0
									  from ls.auction_issue ai, ".TS.".currinstrument curr
									  where curr.id=ai.curr_id and curr.shortname like '". $shortNm ."%' and ai.date0 = ".$pred_date."  order by ai.date0 desc) where ROWNUM <= 1");
                      ...
                      sum(dl_volume*round(dl_price,2))/sum(dl_volume) avg_price, 
                       ...
                       (select sum(d.volume) from ls.auction_deals d where d.order_id=t.rowid) as dl_volume, 
                       (select sum(round(d.price,2)) from ls.auction_deals d where d.order_id=t.rowid) as dl_price, 
                       c.shortname shortname, c.id as curr from ls.auction_orders t, 
                       ".TS.".currinstrument c where c.shortname like '%".$shortNm."%' and t.curr_id=c.id and date0 = ".$pred_date."  order by date0 DESC ) where prof > 0 group by curr";
                  ...
                  ".TS.".currinstrument c,
                  ls.auction_issue i where 
				  c.shortname like '%".$shortNm."%' and t.status != 2 and t.curr_id=c.id and t.price > 0 and t.curr_id = i.curr_id and 
                  ...
                  while(oci_fetch($prev__cenaParse)) 
                  {
                     ...
                     $prev__avg_doh = oci_result($prev__cenaParse,3);
                             ...
                             echo "
                               <tr>
                                 <td class='info'>Объем предложения (в тыс. ".$currencies.")</td>
                               ...
                               <tr>
                                 <td class='info'>Объем спроса (по номиналу, в тыс ".$currencies.")</td>
                               ...
                               <tr>
                                <td class='info'>Объем размещения (по номиналу, в тыс ".$currencies.")</td>
                                ...
                                <tr>
                                    <td>Средневзвешенная цена (".$currencies.")</td>
                 ...
                 sum(dl_volume*round(dl_price,2))/sum(dl_volume) avg_price, 
                  max(prof) from ( select t.price, t.profit as prof, 
                  (select distinct d.volume from ls.auction_deals d where d.order_id=t.rowid and d.volume is not null) as dl_volume, 
                  (select distinct round(d.price,2) from ls.auction_deals d where d.order_id=t.rowid and d.volume is not null) as dl_price, 
                  c.shortname shortname, c.id as curr from ls.auction_orders t, 
                  ".TS.".currinstrument c where c.id = ".$page." and t.curr_id=c.id and date0=".MakeLongDate($date1)." order by shortname ASC,t.price DESC ) group by curr";

	
                  
                    //find cena 
                  $cenaParse = oci_parse($itogi,"select t.price ,t.profit as ord from ls.auction_orders t, ".TS.".currinstrument c,ls.auction_issue i where c.id = ".$page." and t.status != 2 and t.curr_id=c.id and t.price > 0 and t.curr_id = i.curr_id and t.date0=".MakeLongDate($date1)." and i.date0=".MakeLongDate($date1)." order by shortname ASC,t.price DESC");
                 ...
                 $curs_pr = oci_parse($itogi,$qwery_str);
                     ...
                     $doh_by_price = oci_parse($itogi,"select profit from LS.AUCTION_ORDERS t where t.curr_id = ".$page." and DATE0=".MakeLongDate($date1)." and status != 2 and price = (select max(price) from LS.AUCTION_ORDERS t where curr_id = ".$page." and DATE0=".MakeLongDate($date1).")");
					
			                     oci_execute($doh_by_price);
                     if(oci_fetch($doh_by_price)) {
                        $doh_by_maxprice = oci_result($doh_by_price,1);
                     }
                     $prev_doh_by_price = oci_parse($itogi,"select profit from LS.AUCTION_ORDERS t where DATE0=".$pred_date." and status != 2 and price = (select max(price) from LS.AUCTION_ORDERS t where DATE0=".$pred_date.")");
                     ...
                     echo "
                        <tr>
                           <td class='info'>Максимальная цена (в ".$currencies.")</td>
                        ...
                        <tr>
                           <td>Цена отсечения (в ".$currencies.")</td>
...
<button class='btn control' style='float: right;' onclick='print_version();'>Распечатать отчет</button>
  ...
  <span style='font-size:15px;'>Общая информация по допразмещению:</span>
	 ...
	 while (oci_fetch($curs55)){
         ...
         if ($c = ora_logon(OP1,OP2))
         {
            ...
            else
            {
               ...
               to_char(price,'fm999G999G990D00', 'nls_numeric_characters='', ''''') as price1, 
               to_char(profit,'fm999G999G990D00', 'nls_numeric_characters='', ''''') as profit, 
               ( select DISTINCT volume from ls.auction_deals d where d.order_id = t.rowid  and d.volume is not null) as volume,
               date0, time0, acc.code, rn.rusnick, 
               ( select DISTINCT volume * price from ls.auction_deals d where d.order_id = t.rowid and d.volume is not null ) as total,
               ( select DISTINCT  profit  from  ls.auction_deals d  where d.order_id = t.rowid and d.volume is not null) as profit_2
               from ls.auction_orders t, ".TS.".ts_currinstrument_arcts curr, ".TS.".tradeaccount acc, ".TS.".firm f,ls.nicks rn where curr.id = ".$page." and t.curr_id=curr.id and t.conf != 2 and t.idfirm=f.id and date0=".MakeLongDate($date1)." and acc.id=t.account ".$f." and f.nick = rn.nick order by comp desc, price desc");
   ...
   <table class='new-report' border='1'>
         ...
         <th class='info_title'>
            Фактическая стоимость (".$currency.")
         </th>
         <th class='info_title'>
            Цена заявки (".$currency.")
         </th>
